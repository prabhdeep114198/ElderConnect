...
import { GaitClusterPanel } from '../components/fallRisk/GaitClusterPanel';
import { MobilityCoachingCard } from '../components/fallRisk/MobilityCoachingCard';
...
// Coaching is fetched lazily after main data to keep dashboard load fast
const COACHING_LAZY_DELAY = 800; // ms
...
export default function FallRiskDashboard() {
    ...
    // AI Coaching — separate state, loaded lazily
    const [coachingPlan, setCoachingPlan] = useState<MobilityCoachingPlan | null>(null);
    const [loadingCoaching, setLoadingCoaching] = useState(false);
    const [coachingError, setCoachingError] = useState<string | null>(null);

    // Animation for coaching section fade-in
    const coachingFadeAnim = useRef(new Animated.Value(0)).current;

    // ── Data loading ──────────────────────────────────────────────────────

    const fetchCoachingPlan = useCallback(async (uid: string) => {
        if (!uid) return;
        setLoadingCoaching(true);
        setCoachingError(null);
        try {
            const plan = await fallRiskService.getCoachingPlan(uid);
            setCoachingPlan(plan);
            // Fade the coaching section in smoothly
            ...
            console.error('Coaching plan fetch error:', err);
            setCoachingError('Could not load coaching plan. Tap to retry.');
        } finally {
    ...
    const fetchData = useCallback(async () => {
        if (!user) return;
        try {
            // Main data — always fast (no AI call)
            ...
            // Fetch coaching lazily — small delay so the main UI renders first
            setTimeout(() => fetchCoachingPlan(user.id), COACHING_LAZY_DELAY);
        } catch (err: any) {
            console.error('Dashboard fetch error:', err);
            setError(err.message || 'Failed to fetch data');
        }
    }, [user, fetchCoachingPlan]);
    ...
    const onRefresh = useCallback(async () => {
        ...
        coachingFadeAnim.setValue(0);
        await fetchData();
        setRefreshing(false);
    }, [fetchData, coachingFadeAnim]);
        ...
        if (analysis.currentScore > alertThreshold) {
            Notifications.scheduleNotificationAsync({
                content: {
                    ...
                    body: `Risk score is ${Math.round(analysis.currentScore)}, exceeding threshold of ${alertThreshold}.`,
    ...
    if (error) {
        return (
            <View style={[styles.container, { backgroundColor: colors.background, justifyContent: 'center', alignItems: 'center', padding: 20 }]}>
                ...
                <Text style={{ color: colors.text, fontSize: 18, fontWeight: 'bold', marginTop: 10 }}>
                    {t('analysisFailed')}
                </Text>
            ...
            >
                ...
                <View style={styles.meterSection}>
                    ...
                    <View style={[styles.statusBadge, {
                        backgroundColor: analysis.currentScore > 70 ? colors.error + '20'
                            : analysis.currentScore > 40 ? colors.warning + '20'
                            : colors.success + '20',
                    }]}>
                        <Text style={[styles.statusText, {
                            color: analysis.currentScore > 70 ? colors.error
                                : analysis.currentScore > 40 ? colors.warning
                                : colors.success,
                        }]}>
                            {analysis.currentScore > 70 ? t('highRisk')
                                : analysis.currentScore > 40 ? t('moderateRisk')
                                : t('lowRisk')}
                        </Text>
                    </View>
                </View>

                {/* ── AI Gait Cluster Panel (NEW) ───────────────────────── */}
                {analysis.gaitClusters && (
                    <View style={styles.section}>
                        <GaitClusterPanel clusters={analysis.gaitClusters} />
                    </View>
                )}
                ...
                <View style={styles.section}>
                        ...
                        <View style={styles.filterContainer}>
                            {(['7d', '30d', '90d'] as const).map((range) => (
                                <TouchableOpacity
                                    ...
                                    style={[styles.filterButton, timeRange === range && { backgroundColor: colors.primary }]}
                                    accessibilityLabel={`Filter by ${range}`}
                                    accessibilityRole="button"
                                    accessibilityState={{ selected: timeRange === range }}
                                >
                                    <Text style={[styles.filterText, { color: timeRange === range ? '#FFF' : colors.mutedText }]}>
                                        {range.toUpperCase()}
                                    </Text>
                                </TouchableOpacity>
                            ))}
                    ...
                    <RiskTrendChart
                        ...
                        forecastData={analysis.forecasts.map(f => ({ label: `+${f.days}d`, value: f.predictedScore }))}
                    />
                </View>
                ...
                {/* ── AI Mobility Coaching (NEW — lazy loaded) ──────────── */}
                <View style={styles.section}>
                    {loadingCoaching && !coachingPlan && (
                        <View style={[styles.coachingLoader, { backgroundColor: colors.card, borderColor: colors.border }]}>
                            <ActivityIndicator color={colors.primary} />
                            <Text style={[styles.coachingLoaderText, { color: colors.mutedText }]}>
                                Generating your personalized coaching plan…
                            </Text>
                        </View>
                    )}

                    {coachingError && !coachingPlan && (
                        <TouchableOpacity
                            onPress={() => user && fetchCoachingPlan(user.id)}
                            style={[styles.coachingLoader, { backgroundColor: colors.card, borderColor: colors.border }]}
                        >
                    ...
                    {coachingPlan && (
                        <Animated.View style={{ opacity: coachingFadeAnim }}>
                            <MobilityCoachingCard plan={coachingPlan} />
                        </Animated.View>
                    )}
                </View>

                {/* ── Legacy Recommendations (kept as fallback) ─────────── */}
...
const styles = StyleSheet.create({
    ...
    meterSection: { alignItems: 'center', marginBottom: 30, position: 'relative' },
    statusBadge: { marginTop: 20, paddingHorizontal: 20, paddingVertical: 8, borderRadius: 20 },
    ...
    indicatorCard: {
        width: '48%', padding: 15, borderRadius: 16, marginBottom: 15,
        alignItems: 'center', borderWidth: 1, elevation: 2,
        shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 2,
    },
    indicatorVal: { fontSize: 20, fontWeight: 'bold', marginTop: 8 },
    indicatorLabel: { fontSize: 11, fontWeight: '600', marginTop: 2, textAlign: 'center' },
    filterContainer: { flexDirection: 'row', backgroundColor: 'rgba(0,0,0,0.05)', borderRadius: 8, padding: 2 },
    filterButton: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 6 },
    filterText: { fontSize: 12, fontWeight: '600' },
    caregiverSection: { padding: 20, borderRadius: 20, marginTop: 10, marginBottom: 30 },
    thresholdControl: {
        flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
        padding: 15, borderRadius: 12, backgroundColor: 'rgba(0,0,0,0.03)',
    },
    thresholdLabel: { fontSize: 16, fontWeight: '600' },
    thresholdButtons: { flexDirection: 'row' },
    smallBtn: { width: 36, height: 36, borderRadius: 18, justifyContent: 'center', alignItems: 'center' },
    actionButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 15, borderRadius: 12 },
